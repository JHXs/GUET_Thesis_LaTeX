...
% LastEditTime : 2026-01-22 16:28:36 +0800
...
% \documentclass[promaster,pversion]{GUET-Thesis} % 打印版
...
\addbibresource[location=local]{./References/References.bib} % 参考文献数据源加载
\addbibresource[location=local]{./References/第二章参考文献.bib} % 参考文献数据源加载
...
\Title{                                          % 题目{中文}{英文}  
    基于时空特征融合的 Transformer 架构空气质量预测模型研究
}{
    Research on Air Quality Prediction Model Based on Transformer Architecture and Spatiotemporal Feature Fusion
    }                                            % 标题中插入“\\&”命令进行换行
\Author{蒋洪兴}                                     % 作者姓名
\Advisor{凌永发}                                    % 导师姓名
\Protitle{教授}                                   % 导师职称
\School{信息与通信学院}                              % 所在学院
\Major{新一代电子信息技术（含量子技术等）}                                  % 学科专业或领域
\ResearchDirection{无线传感器网络}                   % 研究领域（盲审用）
\DegreeCategories{工学硕士}                       % 申请学位门类或类别
\StudentNumber{2020XXXXX}                        % 学号
\Secrets{\Version}                               % 密级，不涉密请空着 \Version 为模板版本号命令(可以去掉)
