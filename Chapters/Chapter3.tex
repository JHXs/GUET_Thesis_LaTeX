...
\begin{center}
    \begin{talltblr}[
        caption = {复杂定义示例(提供脚注)},
        entry = {复杂定义示例},
        note{1} = {服务器发起的最后一个操作被视为聚合部分},
        ]{
        colspec = {Q[3.3cm,c] Q[3.3cm,c] Q[3.3cm,c]},
        }
        方案 & 参数1 & 参数2 \\
        ...
        本章方案\TblrNote{1} & $n\times$SS & $n\times$P \\
    \end{talltblr}
\end{center}
...
\begin{longtblr}[
    ...
    entry = {RSM仿真实验规划表},
        ...
        leftsep=1pt, % 设置左外侧边距
        rightsep=1pt, % 设置右外侧边距
        rowsep=0pt, % 设置行间距
        colsep=16pt, % 设置每列宽度
        rowhead = 1,
    }
    标准序 & 运行序 & {$H_{rib}\ \mathrm{(mm)}$} & {$H_{pf}\ \mathrm{(mm)}$} & {$N_{pf}$} & {$N_{ac}$} \\
    11  & 1   & 0.16            & 0.8            & 6        & 16       \\
