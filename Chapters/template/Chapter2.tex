...
% LaTeX中图片推荐使用pdf格式。使用Origin可导出矢量无白边的图片以保证清晰度，其次推荐使用jpg，png格式图片。
% 为了保证图片的清晰，jpg图片导出时dpi建议设置为300，png图片导出时建议宽度设置为1024像素（可根据需求自行设置），长度随宽度变化。
% 如果将SVG图转换为PDF后，将上述代码修改为：
% \begin{figure}[htb]
%     \centering
%     \includegraphics[width=0.8 \textwidth]{你的SVG文件名.pdf}
%     \caption[图片标题在目录中显示的内容]{完整的图片标题}
%     \label{fig:your-label}
% \end{figure}
% 正文中引用：如\cref{fig:your-label}所示
